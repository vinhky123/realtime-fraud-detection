locals {
  ...
  private_route_table_ids = var.environment == "prod" ? aws_route_table.private[*].id : [aws_route_table.private[0].id]
  gateway_route_table_ids = concat([aws_route_table.public.id], local.private_route_table_ids)
}
...
# VPC Endpoints (Interface) - ECR, CloudWatch Logs, etc.
